...
\section{Introduction}\label{sintroduction}
 ...
 \cite{wang1996pct} defines the layer $[k,k+\de k]$ of random loss $x\ge 0$ as the amount of $x$ in excess of $k$ with  the excess  capped at $k+\de k$.   Thus the layer is 
 $
 L_{k+\de k} - L_k  
 $
...
\end{equation}
where  $\de L_k$ is the infinitesimally small layer $[k,k+\de k]$ and $(x>k)$ is an indicator function equal to 1 if $x>k$ and 0 otherwise.  Equation \eref{layers} decomposes $x$ into infinitely many comonotonic layers noting $L_k$ is non-decreasing in $x$.  A  proof of \eref{layers} follows by noting the final integral in \eref{layers} is $\int_0^x \de k = x$.
...
\end{equation}
while the mean of layer $[\ell_i,\ell_{i+1}]$ is
$$
...
The following modifies \eref{layers} based on the motivating example in \sref{smotivating}. Consider a continuous random loss $x\geq 0$ with distribution function $F$ and inverse distribution function $F^-$. For $0\le\alpha\le 1$, the $\alpha$-Value-at-Risk (VaR$_\alpha$) of $x$ is  $V_\alpha\equiv F^-(\alpha)$  and 
...
$$
This is the insured loss for a policy with deductible $V_\alpha$ and limit $V_{\beta}$, or the reinsured loss under an excess--of--loss reinsurance arrangement. When capital $V_\alpha$ is held against loss $x$, then $L_\alpha$  is the capital used and  $L_1-L_\alpha$ is the capital shortfall. VaR layers  characterise losses. For example the $L_1-L_\alpha$ layer represents additional losses contributed by the highest $1-\alpha$ portion of possible outcomes.

Endpoints of VaR layers are VaRs or percentiles. VaR endpoints adjust to the shape and scale of the loss distribution. The probability of the loss reaching the $\mathrm{VaR}_\alpha$ layer is fixed at $\alpha$, hence the mean and risk of VaR layers are comparable across loss distributions. In contrast layers indexed on the absolute scale as in \sref{sintroduction} are at different probability thresholds depending on the loss distribution.


From \eref{varlayer}, any random variable $x\ge 0$ can be thought of as a sum of  $\mathrm{VaR}_\alpha$ layers over $0\le\alpha\le 1$. This paper uses \eref{varlayer} to decompose and analyse the mean and risk of $x$ across VaR layers. 
...
$$
R_{[\alpha,\beta]} = \int_\alpha^\beta r_\pi^* m_\pi \de \pi\ .
$$
