...
Consider $100$ ordered observations of an insurance loss $x$: $x_1<x_2<\ldots<x_{100}$. Statistics $x_{i+1}-x_i$ and $(100-i)(x_{i+1}-x_i)$ are respectively called the spacing and normalised spacing in order statistics \citep{shaked2007stochastic}. Spacings are of interest in engineering, for example corresponding to elapsed times between successive  failures of components forming a system.

This paper applies spacings to construct a framework for analyzing mean and risk over various layers of an insurance loss. The analysis yields solutions to common pricing, risk and capital decisions. The framework utilises and develops the following result for the sample mean of $x$:
$$
\bar{x}=   x_1 + \frac{99}{100} (x_2-x_1) +\ldots + \frac{100-i}{100} (x_{i+1}-x_i)+\ldots +\frac{1}{100}(x_{100}-x_{99})  \;,
$$
the sum of normalized spacings divided by the sample size. The term
$$
m_i = (1-i/100) (x_{i+1}-x_i)
$$
is the mean of the layer $[x_i,x_{i+1}]$ equal to $0$ for $x_1,\ldots,x_i$ and $x_{i+1}-x_i$ for $x_{i+1},\ldots,x_n$. A layer is a portion of every loss outcome crossing an interval. Hence $m_i$ is the spacing $x_{i+1}-x_i$ multiplied by the proportion $1-i/100$ of observations exceeding $x_i$. Adding up all layer means yields the overall mean $\bar{x}$.

Loss layers are important quantities in insurance and reinsurance, particularly for property and casualty. Typical insurance and excess-of-loss reinsurance cover the loss layer between an excess and a limit. Capital shortfall is the loss layer starting from available capital. Layer means, and risks, are hence critical inputs to pricing, capital and risk decisions. \cite{salzmann1963rating}, \cite{evans2001exposure} and \cite{wang1996pct} further discuss loss layers.


The following are important observations of the above setup:

\bi

\i Two opposing factors drive $m_i$ as $i$ increases. The probability $1-i/100$ decreases, since a smaller proportion of the sample reaches higher layers. For a right tailed loss distribution, the spacing $x_{i+1}-x_i$ increases with $i$, with greater skewness yielding larger spacings. The overall behaviour of $m_i$ characerises the shape of the loss distribution.

\i End points of loss layers are expressed using sample values representing empirical percentiles or values-at-risk (VaRs) of $x$. Hence the top 5 loss layer covers losses above the $95$th percentile, for example. In contrast, current literature expresses loss layers in absolute amounts, for example between 1 and 2 million dollars. VaR layers are considered in this paper as they adjust to the shape and scale of the loss distribution, and can be interpreted probabilistically.
...
\i The risk-adjusted layer mean replaces the exceedance probability $1-i/100$ in $m_i$ with a distorted probability $1-\Phi(i/100)>1-i/100$, yielding
$$
\tilde{m}_i = \left\{1-\Phi(i/100)\right\} (x_{i+1}-x_i) > m_i
$$
Distorted probabilities introduce risk considerations when assessing layer means, and are applied in distortion risk measures \citep{wang1996pct} and cumulative prospect theory \citep{tversky1992apt}.

\ei
Layer means and risks form a framework to optimise pricing, capital and risk decisions. Focus is usually placed on layers with large mean or risk. The framework provides elegant solutions and insights to insurance and also financial problems which may be difficult to express and solve using existing statistical methods.
...
\section{VaR layers and mean densities}\label{smeansetup}

This paper formalises and develops the example in section \aref{sintroduction} for a continuous random loss $x\geq 0$ with distribution function $F$. Similar results apply to discrete losses. Define the $\alpha$-Value-at-Risk (VaR) of $x$ as $V_\alpha\equiv F^-(\alpha)$ for $0\leq\alpha\leq 1$, where $F^-$ is the inverse distribution function of $x$.

Decompose $x$ into the integral
\begin{equation}\label{varlayer}
x=\int_0^1 (x>V_\alpha)\de V_\alpha =  \int_0^1 x_\alpha \de \alpha \cq x_\alpha\equiv (x>V_\alpha)V_\alpha'\;,
\end{equation}
where $(x>V_\alpha)$ is an indicator function equal to one if $x>V_\alpha$ and zero otherwise, and $V_\alpha'$ is the derivative of $V_\alpha$ with respect to $\alpha$. Call $x_\alpha \de\alpha$ where $\de\alpha \sim 0$ the $\alpha$-VaR layer of $x$. The $\alpha$-VaR layer is the portion from $V_\alpha$ to $V_{\alpha+\de\alpha}$ of $x$ if $x>V_\alpha$, and corresponds to the layer $[x_i,x_{i+1}]$ in section \aref{sintroduction}. The $\alpha$-VaR layer has width $V_{\alpha+\de\alpha}-V_\alpha \sim V_\alpha'\de\alpha$ where the derivative $V_\alpha'$ is called VaR-spacing, corresponding to spacing $x_{i+1}-x_i$ in order statistics.

From \eref{varlayer}, $x$ is formed by adding $\alpha$-VaR layers over $0\le\alpha\le 1$. This paper applies \eref{varlayer} to decompose the mean and risk of $x$ across VaR layers. As noted in section \aref{sintroduction}, layers are important as they represent insurance and reinsurance coverages, and their mean and risk values are critical inputs to pricing, capital and risk management decisions.

Endpoints of VaR layers are VaRs or percentiles. VaR endpoints are used in this paper as they adjust to the shape and scale of the loss distribution. The probability of the loss reaching the $\alpha$-VaR layer is fixed at $\alpha$, hence the mean and risk of a VaR layer are comparable across loss distributions. \cite{wang1996pct} consider layers indexed on the absolute scale using the decomposition:
$$
x=\int_0^1 (x>k) \de k \;,
$$
hence the $k$-layer of $x$ is $(x>k)\de k$. The probability of reaching the $k$-layer is $1-F(k)$ and depends on $F$. As a result the mean and risk of the $k$-layer are not comparable across loss distributions.



The mean density indicates the mean of various VaR layers:
\begin{equation}\label{meancont}
m_\alpha \equiv \E(x_\alpha)=\E\left\{(x>V_\alpha)V_\alpha' \right\}
=(1-\alpha)V_\alpha'
\end{equation}
over $0\le\alpha\le 1$, where $\E$ calculates expectations under the distribution function $F$. The mean density $m_\alpha$ is the product of $1-\alpha$, the probability of $x$ reaching the $\alpha$-VaR layer, and VaR-spacing $V_\alpha'$ representing layer width. As $\alpha$ increases, $1-\alpha$ decreases reflecting reduced likelihood of reaching higher layers. On the other hand $V_\alpha'$ increases for a right-tailed loss distribution where the probability density is decreasing and hence spacings between successive VaRs increase. The behaviour of the mean density depends on the how $V_\alpha'$ moves relative to $1-\alpha$ and is hence characterised by the loss distribution. For example, the exponential distribution has a constant mean density whereas the Pareto distribution has an increasing mean density due to more rapidly increasing VaR spacings. Example mean densities are shown in section \aref{smeaneg}.

Using \eref{varlayer}, integrating the mean density $m_\alpha$ over all $0\le\alpha\le1$ yields the overall mean of $x$:
$$
\int_0^1 m_\alpha \de \alpha = \E\left( \int_0^1 x_\alpha \de \alpha \right) = \E(x) \;.
$$
Integrating $m_\alpha$, and $x_\alpha$, over subsets of the unit interval yields mean values and values of layers representing for example insurance coverage with a deductible and limit in place. This result is discussed in the next section.


As mentioned above in this section, \cite{wang1996pct} considers loss layers on the absolute scale and the $k$-layer is $(x>k)\de k$. The corresponding mean density is $\E\{(x>k)\}=S(k)$ where $S\equiv 1-F$ is the survival function. \cite{wang1996pct} calls $S$ the premium layer density.



\section{Integrating VaR layers and mean densities}


Integrating $\alpha$-VaR layers over the interval $[a,b]$ yields the $[a,b]$-VaR layer, the portion from $V_a$ to $V_b$ of the loss:
$$
X_{[a,b]} \equiv \int_a^b x_\alpha \de\alpha = \int_a^b (x>V_\alpha)\de V_\alpha =   (V_a< x\leq V_b)(x-V_a)
$$
$$
+ (x>V_b)(V_b-V_a)
=  \max\left\{ \min(x,V_b)-V_a,0\right\}    \;.
$$
If $a=0$ then $X_{[0,b]}=\min(x,V_b)$ is the portion of $x$ up to $b$-VaR. If $b=1$ then $X_{[a,1]}=\max(x-V_a,0)$ is the portion of $x$ in excess of $a$-VaR. Note
\begin{equation}\label{differentiate}
\frac{\partial}{\partial b} X_{[a,b]} = x_b \cq \frac{\partial}{\partial a} X_{[a,b]} = -x_a \;.
\end{equation}
VaR layers are important insurance quantities. For example the $[a,b]$-VaR layer is the insured loss for a policy with deductible $V_a$ and limit $V_b$, or the reinsured loss under an excess-of-loss reinsurance arrangement. When capital $V_a$ is held against loss $x$, the $[a,1]$-VaR layer is the capital shortfall and the $[0,a]$-VaR layer is the capital used. VaR layers also characterise losses of a certain nature. For example the $[a,1]$-VaR layer represents additional losses contributed by the highest $(1-a)$ portion of possible outcomes.

Integrating the mean density over any interval yields the mean of the VaR layer over the same interval:
$$
M_{[a,b]} \equiv \int_a^b m_\alpha \de\alpha = \E\left(\int_a^b x_\alpha \de\alpha\right)=\E(X_{[a,b]}) \;.
$$
and similar results as \eref{differentiate} apply to $M_{[a,b]}$:
$$
\frac{\partial}{\partial b} M_{[a,b]} = m_b \cq \frac{\partial}{\partial a} M_{[a,b]} = -m_a \;.
$$
$M_{[a,b]}$ represents for example the mean covered loss or pure premium under an insurance or reinsurance arrangement with deductible $V_a$ and limit $V_b$, and the expected capital shortfall $(a=c,b=1)$ or capital usage $(a=0,b=c)$ when capital $V_c$ is held against $x$. $M_{[a,b]}$ over various values of $a$ and $b$ is hence an important input to insurance and reinsurance pricing, and decisions on policy conditions, capital amounts to hold, and other risk and capital management strategies. Section \aref{srisksetup} discusses risk values of VaR layers, also a critical input to these decisions when volatility is of concern. Applications of mean and risk densities are discussed in sections \aref{sapplication} and beyond.


\section{Example mean densities}\label{smeaneg}


The following are mean densities for common probability distributions. In all cases $\beta>0$ is a scale parameter.

\bi

\i Consider the uniform distribution over $[0,\beta]$. Then
$$
V_\alpha=\beta\alpha \cq V_\alpha'=\beta \cq m_\alpha=\beta(1-\alpha) \;,
$$
therefore the mean density is linearly decreasing. Spacings are equal over all VaR layers, and losses are less likely to reach higher layers, hence the mean density is decreasing. The mean density is also proportional to scale, and this is case in general.


\i Consider the exponential loss distribution with mean $\beta$. Then
$$
F(x)=1-\e^{-x/\beta} \cq V_\alpha=-\beta \log(1-\alpha) \cq V_\alpha'=\frac{\beta}{1-\alpha} \cq m_\alpha=\beta\;,
$$
hence the mean density is constant. VaR layers have constant mean since increasing VaR spacings exactly offset decreasing probabilities of reaching higher layers.

\i Consider the Pareto distribution with shape parameter $\gamma>1$:
$$
F(x)=1- \left(\frac{\beta}{\beta+x}\right)^\gamma \cq V_\alpha= \beta\left\{(1-\alpha)^{-1/\gamma}-1\right\} \;,
$$
$$
 V_\alpha'=\frac{\beta}{\gamma (1-\alpha)^{1/\gamma+1}} \cq m_\alpha = \frac{\beta}{\gamma (1-\alpha)^{1/\gamma}} \;.
$$
Mean density increases to infinity for higher VaR layers. Due to the high skewness of the Pareto distribution relative to uniform and exponential, increasing VaR spacings more than offset decreasing probabilities.

\i For the Weibull distribution with shape parameter $k>0$,
$$
F(x)=1-\e^{-(x/\beta)^k} \cq V_\alpha = \beta\left\{- \log(1-\alpha)\right\}^{1/k} \;,
$$
$$
V_\alpha'=\frac{\beta}{k(1-\alpha)}\left\{-\log(1-\alpha)\right\}^{1/k-1}  \cq
m_\alpha=\frac{\beta}{k}\left\{-\log(1-\alpha)\right\}^{1/k-1} \;.
$$
Setting $k=1$ yields the exponential distribution and $m_\alpha=\beta$. Reducing $k$ increases skewness and yields increasing $m_\alpha$. Vice versa if $k>1$.
...
\begin{figure}
  \begin{center}
    ...
    \end{tabular}
    \caption{Mean densities for uniform ($\beta=2$), exponential ($\beta=1$), Pareto ($\beta=0.5$, $\gamma=1.5$) and Weibull ($\beta=1.13$, $k=2$) distributions. The mean densities all integrate to $1$.}
    \label{fmeancont}
  \end{center}
\end{figure}
...
The mean density is non-negative for any loss $x\geq 0$. In addition the mean density is proportional to scale: the mean density of $kx$ where $k>0$ is constant is $km_\alpha$. Further consider an increasing loss transformation $y=\gamma(x)\geq 0$. Then  $y$ has $\alpha$-VaR $\gamma(V_\alpha)$ and hence mean density  $(1-\alpha)\gamma'(V_\alpha)V_\alpha'=m_\alpha\gamma'(V_\alpha)$.
...
$$
Taking the inverse of the result yields the loss distribution $F$. Hence $\alpha$-VaR is the integral of scaled mean density $m_t$ up to $t=\alpha$ with scaling factor $1/(1-t)$. Higher values of $m_t$, especially over $t$ closer to $\alpha$, implies higher $\alpha$-VaR.



The mean density at $\alpha$-VaR layer of $x$ is the reciprocal of the hazard rate at $\alpha$-VaR:
$$
m_\alpha = \left(\lambda_{V_\alpha}\right)^{-1}  \cq \lambda_x\equiv \frac{f(x)}{1-F(x)} \;,
$$
where $f\equiv F'$ is the loss density. The result follows by noting $F(V_\alpha)=\alpha$ and $V_\alpha'=1/f(V_\alpha)$. The hazard rate at say $x_0$ indicates the probability of $x=x_0$ given $x>x_0$. \cite{hogg2009loss} further discusses hazard rates.
...
r_\alpha \equiv \Ex(x_\alpha)-\E(x_\alpha) = \{1-\Phi(a)\}V_\alpha'-(1-\alpha)V_\alpha'=\{\alpha-\Phi(\alpha)\}V_\alpha'
\end{equation}
...
Also note end points of VaR layers are unchanged at $V_\alpha=F^-(\alpha)$, even though the $\alpha$-VaR of the distorted distribution is $F^-\{\Phi^-(\alpha)\}$. Therefore end points of VaR layers are fixed and specified using the original distribution, and distortion only affects the probability of the loss reaching various layers.

Similar to $m_\alpha$, integrating $r_\alpha$ over all $\alpha$ yields the distortion risk of $x$:
$$
\int_0^1 r_\alpha \de \alpha = \int_0^1 \{\Ex(x_\alpha)-\E(x_\alpha)\} \de \alpha = \Ex(x)-\E(x) \;.
$$
Alternatively, $\alpha$-VaR layers for all $\alpha$ are comonotonic, and distortion risks of comonotonic random variables are additive \citep{wang1997aci}. In addition integrating the risk density over $[a,b]$ yields the risk of the $[a,b]$-VaR layer:
$$
R_{[a,b]} \equiv \int_a^b r_\alpha \de \alpha = \Ex\left(X_{[a,b]}\right)-\E\left(X_{[a,b]}\right)  \;.
$$
For example $R_{[a,b]}$ is the risk of an insured loss $x$ with deductible $V_a$ and limit $V_b$. The premium for this insured loss is then $X_{[a,b]}+\theta R_{[a,b]}$ where $\theta>0$ is a profit margin.  $R_{[0,b]}$ is the risk of losses capped at $V_b$, and $R_{[a,1]}$ is the risk of losses in excess of $V_a$. These two risks are important inputs when setting capital for example. The risk density spreads the overall risk of $x$ across VaR layers and focus is generally placed on higher risk layers to form optimal decisions. Applications of mean and risk densities are further discussed in section \aref{sapplication}.
...
Given individual risk ratios, the overall risk and risk ratio of any VaR layer $X_{[a,b]}$ is computed as
$$
...
\i Assume the power distortion operator $\Phi(v)=v^n$ where $n\geq 1$. If $n$ is an integer, the distortion risk is $\E\{\max(x_1,\ldots,x_n)\}-\E(x)$ or the expected-maximal-loss in excess of the mean loss, where $x_1,\ldots x_n$ are independent copies of $x$. The risk ratio in this case is
$$
...
The volatility density is the standard deviation of $x_\alpha$:
\begin{equation}\label{volatility}
s_\alpha \equiv \sqrt{\var(x_\alpha)} = \sqrt{\alpha(1-\alpha)}V_\alpha' \;,
\end{equation}
since the $\alpha$-VaR layer is Bernoulli distributed with probability $1-\alpha$ and scale $V_\alpha'$. The volatility ratio is the ratio between volatility and mean densities:
$$
s_\alpha^* \equiv \frac{s_\alpha}{m_\alpha} = \frac{\sqrt{\alpha(1-\alpha)}V_\alpha'}{(1-\alpha)V_\alpha'} = \sqrt{\frac{\alpha}{1-\alpha}} \;,
$$
and is the coefficient of variation of $x_\alpha$. Similar to risk ratios, volatility ratios are increasing and are independent of the loss distribution. Volatility ratio starts at zero at the bottom VaR layer, and increases to infinity at the top VaR layer.
...
\sqrt{\var\left( X_{[a,b]}\right)} \le S_{[a,b]}  \;,
\end{equation}
since the standard deviation of a sum ($X_{[a,b]}$) is less than the sum of standard deviations ($S_\alpha$). VaR layers are comonotonic but not linearly dependent, hence equality in \eref{bound} does not hold in general.
...
Previous sections define mean, risk and volatility densities for a continuous loss random variable. It is common for insurance and financial companies to simulate future outcomes to make decisions. Given an ordered sample of simulated losses $x_1,\ldots,x_n$, calculate mean, risk and volatility densities as follows:
\bi
\i Set VaRs equal to ordered observations: $V_{i/n}=x_i$. The $i/n$-VaR layer is $(x>x_i)(x_{i+1}-x_i)$ where $x$ is random.
...
$$
Taking sums over other subsets yield mean, risk and volatility over corresponding VaR layers. Manipulating the first summation above yields the arithmetic mean $\sum_{i=1}^n x_i /n$.
...
The following applies mean and risk densities to pricing loss layers. Given loss $x$, suppose the layer $X_{[a,b]}$ is insured (or reinsured). Hence $V_a$ is the deductible or excess and $V_b$ is the limit. The payout is $x-V_a$ if $x>V_a$, and is capped at $V_b-V_a$. The premium is
$$
...
Suppose an insurer faces loss $x$, and enters into a reinsurance arrangement whereby a portion $t_\alpha$ of every $\alpha$-VaR layer is reinsured and the remaining portion $1-t_\alpha$ is retained. Hence $\alpha$-VaR layer and its mean and risk all reduce by $t_\alpha$. The retained loss and the mean and risk densities are, respectively,
...
$$
p(c)= \left\{ \pi V_c + \E\left(x_{[0,c]}\right)  \right\} + \left\{ \int_c^1 r_\alpha \de \alpha +  \E\left(x_{[c,1]}\right) \right\}
