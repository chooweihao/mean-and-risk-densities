...
\section{Introduction}\label{sintroduction}
 ...
 \cite{wang1996pct} defines the layer $[k,k+\de k]$ of random loss $x\ge 0$ as the amount of $x$ in excess of $k$ with  the excess  capped at $\de k$.   Thus the layer is
 $
 L_{k+\de k} - L_k
 $
 where $L_k \equiv \min(x,k)$.
Then $x$ is the sum of layers covering $[0,\infty]$ as illustrated in \fref{flayer}. Letting $\de k\rightarrow 0$ yields
\begin{equation}\label{layers}
x= \int_0^\infty \de L_k = \int_0^\infty L_k' \de k = \int_0^\infty (x>k) \de k \;,
\end{equation}
where  $\de L_k$ is the infinitesimally small layer $[k,k+\de k]$ and $(x>k)$ is an indicator function equal to 1 if $x>k$ and 0 otherwise. A  proof of \eref{layers} follows by noting the final integral in \eref{layers} is $\int_0^x \de k = x$. 

Equation \eref{layers} decomposes $x$ into infinitely many layers. Each layer is a random variable and is comonotonic with all other layers since $L_k$ is non-decreasing in $x$. Integrating these layers over an arbitrary interval yields a larger layer:
$$
\int_a^b \de L_k = L_b-L_a = \min(x,b)-\min(x,a) \;.
$$  
If $x$ has distribution function $F$, the mean of layer $\de L_k $ is $\{1-F(k)\} \de k$. \cite{wang1996pct} calls $1-F$ the layer premium density of $x$ as it identifies the insurance premium of each layer and integrates to the overall mean: $\int_0^\infty \{1-F(k) \}\de k = \E(x)$ where $\E$ is the expectation. In addition \cite{wang1996pct} distorts the layer premium density by replacing original probabilities with distorted probabilities. The distorted layer premium density delivers premiums with a risk loading.

\begin{figure}
  \begin{center}
    \begin{tabular}{cc}
      \resizebox{80mm}{!}{\includegraphics{layer.eps}}
    \end{tabular}
    \caption{Any $x$ is the sum of layers $[k,k+\de k]$ covering $[0,\infty]$. The grey shaded area represents values of layers counted in $x$. }
    \label{flayer}
  \end{center}
\end{figure}


Loss layers are important insurance and reinsurance quantities, particularly for property and casualty. Insurance and excess--of--loss reinsurance typically cover the loss layer between an excess and a limit. Capital shortfall is the loss layer starting from available capital. Layer means, and risks, are  critical inputs to pricing, capital and risk decisions.

This paper extends results  in \cite{wang1996pct} by defining layers on the percentile or value-at-risk (VaR) scale. VaRs capture  the shape and scale of the loss distribution, and hence provide comparability across loss distributions. VaRs are standard constructs in insurance and financial markets. For example Solvency II insurance regulation applies $90\%$ and $99.5\%$ VaRs \citep{eling2007solvency} in capital requirements. Banking regulations Basel II and more recently Basel III also reference VaRs in risk measurement \citep{chernobai2008operational}.

The development in this paper yields a framework to form optimal pricing, capital and risk decisions. Focus is usually  on layers with large mean or risk. The framework yields solutions and insights to insurance and financial problems which are difficult or awkward to  express and solve using  standard statistical approaches.
...
Whilst \eref{layers} provides the key initial mathematical result, the following example provides further motivation for the  developments in this paper.
...
The following modifies the layer decomposition of $x$ in \eref{layers} using the motivating example in \sref{smotivating}. Consider a continuous random loss $x\geq 0$ with distribution function $F$ and inverse distribution function $F^-$. For $0\le\alpha\le 1$, the $\alpha$-Value-at-Risk (VaR$_\alpha$) of $x$ is  $V_\alpha\equiv F^-(\alpha)$  and
\begin{equation}\label{varlayer}
x=\int_0^1 \de L_\alpha = \int_0^1  L'_\alpha \de \alpha = \int_0^1 (x>V_\alpha)V_\alpha' \de \alpha \cq
L_\alpha  \equiv \min(x,V_\alpha)  \;,
\end{equation}
where $L_\alpha$ is  indexed on VaR thresholds or exceedance probabilities instead of absolute loss amounts as in \eref{layers}. This definition of $L_\alpha$ is applied to remaining developments in this paper. Then $\de L_\alpha$ represents the VaR$_\alpha$ layer $[V_\alpha,V_{\alpha+\de\alpha}]$, and corresponds to layer $[\ell_{\alpha n},\ell_{\alpha n+1}]$ in \sref{smotivating} assuming $n\rightarrow \infty$. The width of the VaR$_\alpha$ layer is $V_{\alpha+\de\alpha}-V_\alpha \approx V_\alpha' \de \alpha$ and $V_\alpha'$ is called the VaR--spacing,  the continuous analogue to the discrete spacing $\ell_{i+1}-\ell_i$.


Endpoints of VaR layers self-adjust to the shape and scale of the loss distribution. The probability of the loss reaching the VaR$_\alpha$ layer is fixed at $\alpha$, hence the mean and risk of VaR layers are comparable across loss distributions. In contrast layers indexed on the absolute scale as in \sref{sintroduction} are at different probability thresholds depending on the loss distribution.



The VaR$_{[\alpha,\beta]}$ layer of $x$ is the random variable
$$
\int_\alpha^\beta \de L_\pi = L_\beta-L_\alpha = \min(V_\beta,x)-\min(V_\alpha,x) \;.
$$
This is for example the insured loss for a policy with deductible $V_\alpha$ and limit $V_{\beta}$, or the reinsured loss under an excess--of--loss reinsurance arrangement. When capital $V_\alpha$ is held against loss $x$, then $L_\alpha$  is the capital used and  $L_1-L_\alpha$ is the capital shortfall. VaR layers  characterise losses. For example the $L_1-L_\alpha$ layer represents additional losses contributed by the highest $1-\alpha$ portion of possible outcomes.


From \eref{varlayer}, any random variable $x\ge 0$ can be thought of as a sum of  $\mathrm{VaR}_\alpha$ layers over $0\le\alpha\le 1$. This paper uses \eref{varlayer} to decompose and analyse the mean and risk of $x$ across VaR layers. The analysis yields solutions and insights to common pricing, risk and capital problems. 
...
\end{equation}
where $\E$ is the expectation. The mean density $m_\alpha$ is the product of $1-\alpha$, the probability of $x$ reaching the $\mathrm{VaR}_\alpha$ layer, and the VaR-spacing $V_\alpha'$, measuring layer width.
...
Using \eref{varlayer},
$$
\E(x)= \E\left( \int_0^1 \de L_\alpha \right) =\int_0^1 m_\alpha \de \alpha \;.
$$
Integrating $m_\alpha$ over subsets of the unit interval yields mean values and values of layers representing for example insurance coverage with a deductible and limit:
\begin{equation}
\int_\alpha^\beta m_\pi\de \pi=\E\left( \int_\alpha^\beta \de L_\pi \right)=\E(L_\beta-L_\alpha)  \;,
\end{equation}
 representing, for example, the mean covered loss or pure premium under an insurance or reinsurance arrangement with deductible $V_\alpha$ and limit $V_\beta$, and the expected capital shortfall $(\alpha=\gamma,\beta=1)$ or capital usage $(\alpha=0,\beta=\gamma)$ when capital $V_\gamma$ is held against $x$.   Section \aref{srisksetup} discusses risk values of VaR layers, also a critical input to these decisions when volatility is of concern. Applications of mean and risk densities are discussed in  \sref{sapplication} and beyond.
...
The constant mean density for the exponential distribution provides a benchmark for assessing skewness of the loss distribution. A decreasing mean density (for example uniform distribution and Weibull distribution for $k>1$) implies VaR spacings increase at a lower rate compared to the exponential, and hence lower skewness. Vice versa for an increasing mean density (for example Pareto distribution and Weibull distribution for $k<1$). The derivative of the mean density is
$$
m'_\alpha=(1-\alpha)V_\alpha''-V_\alpha' \;.
$$
Hence the mean density is increasing if and only if $(1-\alpha)V_\alpha''/V_\alpha'>1$.


\section{Properties of mean densities}\label{prop}

The mean density $m_\alpha$ defined in \eref{meancont} is non-negative for any loss $x\geq 0$. Further  if $y=g(x)$ where $g>0$ is  increasing  then
$$
V_\alpha(y)=g\{V_\alpha(x)\}\cq m_\alpha(y) = (1-\alpha)g'\{V_\alpha(x)\}V_\alpha'=m_\alpha(x) g'\{V_\alpha(x)\}\ .
$$
In particular if $y=cx$ then $m_\alpha(y)=cm_\alpha(x)$.
...
If $f$ is the density corresponding to the distribution $F$ then the
$$
...
\section{Risk associated with layers}\label{srisksetup}

This section discusses ``risks" associated with loss layers, where risk is measured using distorted probabilities. Means and risks are critical inputs to decision making for pricing, capital and risk management.  Distortion risks are important inputs to decisions where volatility is of concern. For example distortion risks form premium loadings and capital buffers on insurance losses, and risk premiums on investment returns. Mean values are insufficient as they do not capture the spread and skewness of the distribution of outcomes. Applying various distortion operators generates different distortion risk measures. Examples are discussed in \cite{wang1995insurance}, \cite{wang2000cdo} and \cite{choo2009loss}, and include the proportional hazards risk, conditional-tail-expectation and expected-maximal-loss.

\cite{wang1996pct} distorts the distribution function $F$ with a distortion $\Phi$ to arrive at the distorted distribution function $\Phi\circ F$.  Here  $\Phi$ is an increasing, convex distortion operator satisfying $\Phi(0)=0$ and $\Phi(1)=1$. The distorted risk of $x$ is
$$
...
$$
where  $u\equiv F(x)$.
which is the loss aversion premium \citep{choo2009loss}, where $\cov$ and $\cor$ compute covariance and correlation under $F$, and $\sigma_x$ and $\sigma_\phi$ are standard deviations.

Since $x=\int_0^1\de L_\alpha$, the distorted risk
 of a  VaR layer  is  the difference between distorted and original mean values:
...
\end{equation}
This result follows since the distorted probability of $x>V_\alpha$ is $1-\Phi\{F(V_\alpha)\}=1-\Phi(\alpha)>1-\alpha$ since $\Phi$ is convex. Hence distortion increases the probability of $x$ reaching a VaR layer, and the risk density is proportional to the amount of increase. The increase $\alpha-\Phi(\alpha)$ represents the extent of distortion at $\alpha$.

The end points of VaR layers are unchanged at $V_\alpha=F^-(\alpha)$, even though the $\mathrm{VaR}_\alpha$ of the distorted distribution is $F^-\{\Phi^-(\alpha)\}$. Therefore end points of VaR layers are fixed and specified using the original distribution, and distortion only affects the probability the loss reaching various layers.

Integrating $r_\alpha$ over all $\alpha$ yields the distorted risk of $x$:
$$
\int_0^1 r_\alpha \de \alpha = \int_0^1 \{\Ex(L'_\alpha)-\E(L'_\alpha)\} \de \alpha = \Ex(x)-\E(x) \;.
$$
Alternatively, $\mathrm{VaR}_\alpha$ layers for all $\alpha$ are comonotonic, and distortion risks of comonotonic random variables are additive \citep{wang1997aci}. The distorted risk of the $\mathrm{VaR}_{[\alpha,\beta]}$ layer is
$$
R_{[\alpha,\beta]} \equiv \int_\alpha^\beta r_\pi \de \pi = \Ex\left(L_\alpha-L_\beta\right)-\E\left(L_\alpha-L_\beta\right)  \;,
$$
The premium for the insured loss is  $\E(L_\alpha-L_\beta)+\theta R_{[\alpha,\beta]}$ where $\theta>0$ is a profit margin.  Further$R_{[0,\beta]}$ is the risk of losses capped at $V_\beta$, and $R_{[\alpha,1]}$ is the risk of losses in excess of $V_\alpha$. These two risks are important inputs when setting capital for example. The risk density $r_\alpha$ spreads the overall risk of $x$ across VaR layers.  Generally there is more focus  on higher risk layers when making  decisions. Applications of mean and risk densities are further discussed in \sref{sapplication}.
...
The risk ratio at percentile $\alpha$ is the ratio between risk and mean densities:
...
Given the risk ratios $r^*_\alpha$, the overall risk and risk ratio of a VaR layer is
$$
...
\i Assume  $\Phi(v)=(v>c)(v-c)/(1-c)$ where $0\leq c\leq 1$. The overall distortion risk of $x$ is the conditional-tail-expectation $\E(x|x>V_c)-\E(x)$, the expected value of losses above $c$-VaR in excess of the mean loss. The risk ratio is
$$
